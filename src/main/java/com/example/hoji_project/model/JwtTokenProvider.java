...
public class JwtTokenProvider {
  ...
  // application.yml 에서 secret 값을 가져와서 key에 저장
  ...
  public JwtToken generateToken(Authentication authentication) {
    ...
    String accessToken =
        Jwts.builder().setSubject(authentication.getName()).claim("auth", authorities)
  ...
  public Authentication getAuthentication(String accessToken) {
    ...
    // 클레임에서 권한 정보 가져오기
    Collection<? extends GrantedAuthority> authoritles =
        Arrays.stream(claims.get("auth").toString().split(",")).map(SimpleGrantedAuthority::new)
            .collect(Collectors.toList());

    // UserDetails 객체를 만들어서 Authentication Return
    UserDetails principal = new User(claims.getSubject(), "", authoritles);
    return new UsernamePasswordAuthenticationToken(principal, "", authoritles);
